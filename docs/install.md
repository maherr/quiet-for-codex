...
```shell
curl -fsSL https://raw.githubusercontent.com/maherr/quiet-for-codex/quiet-v0.146.0-beta.4/scripts/release/install.sh | sh
```

### Windows PowerShell

```powershell
& ([scriptblock]::Create((irm -UseBasicParsing https://raw.githubusercontent.com/maherr/quiet-for-codex/quiet-v0.146.0-beta.4/scripts/release/install.ps1)))
```
...
`CODEX_QUIET_RELEASE` to a version such as `0.146.0-beta.4`. The Unix installer
also accepts `--release 0.146.0-beta.4` when downloaded and run as a file.
...
gh attestation verify PATH_TO_ARCHIVE \
  ...
  --source-ref refs/tags/quiet-v0.146.0-beta.4 \
