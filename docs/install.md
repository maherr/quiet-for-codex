...
`CODEX_QUIET_RELEASE` to a version such as `0.146.0-beta.3`. The Unix installer
also accepts `--release 0.146.0-beta.3` when downloaded and run as a file.
...
gh attestation verify PATH_TO_ARCHIVE \
  ...
  --source-ref refs/tags/quiet-v0.146.0-beta.3 \
