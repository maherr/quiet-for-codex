...
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   Quiet always shows its local success page after a successful callback. The
   `useHostedLoginSuccessPage` and `appBrand` fields are accepted for protocol compatibility but
   ignored, so clients cannot redirect the login flow into the stock Codex Desktop handoff.
