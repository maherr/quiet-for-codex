...
fn external_agent_config_migration_selection_excludes_bullets_and_indentation() {
    ...
    assert_eq!(started_prefixes, vec![2, 2, 2, 4, 4, 4, 4, 4, 4]);
    assert!(started_text.starts_with("Import started."));
    ...
    assert_eq!(finished_prefixes, vec![2, 2, 4, 4, 4, 2]);
    assert!(finished_text.starts_with("Import finished:"));
