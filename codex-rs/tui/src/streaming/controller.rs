...
impl StreamCore {
    ...
    fn finalize_remaining(
        ...
        let rendered_start = self.emitted_stable_len.min(rendered.len());
        let selection_fragment = self
            .build_selection_projection_for(&source, &rendered)
            .and_then(|selection| {
                Self::selection_fragment_from_projection(
                    &selection,
                    rendered_start..rendered.len(),
                    rendered.len(),
                )
            });
        let remaining = rendered.split_off(self.emitted_stable_len.min(rendered.len()));
        (remaining, source, selection_fragment)
    }
    ...
    fn selection_fragment_from_projection(
        selection: &StreamSelectionProjection,
        rendered_range: Range<usize>,
        rendered_len: usize,
    ) -> Option<StreamSelectionFragment> {
        let logical_start = rendered_range.start.min(rendered_len);
        let logical_end = rendered_range.end.min(rendered_len);
    ...
    fn current_tail_selection_fragment(&self) -> Option<StreamSelectionFragment> {
        ...
        self.selection_fragment(self.enqueued_stable_len..self.render.lines.len())
    }

    fn retained_live_source(&self) -> Option<&str> {
        self.surface
            .is_retained()
            .then_some(self.state.collector.committed_source())
    ...
    fn has_tail(&self) -> bool {
        if self.surface.is_retained() {
            self.retained_live_source().is_some()
        } else {
    ...
    fn build_selection_projection(&self) -> Option<StreamSelectionProjection> {
        self.build_selection_projection_for(
            self.state.collector.committed_source(),
            &self.render.lines,
        )
    }

    fn build_selection_projection_for(
        &self,
        source: &str,
        rendered_lines: &[HyperlinkLine],
    ) -> Option<StreamSelectionProjection> {
        if source.is_empty() || rendered_lines.is_empty() {
            return None;
        }
        let body_width = self
            .width
            ...
            .unwrap_or(u16::MAX)
            .max(/*other*/ 1);
        let display_lines = visible_lines(rendered_lines.to_vec());
        let projection = match self.render_mode {
            HistoryRenderMode::Rich => {
                let normalized = crate::markdown::unwrap_markdown_fences(source);
        ...
        if row_offsets.last().copied() != Some(projection.rows().len()) {
            tracing::debug!(
                rendered_lines = rendered_lines.len(),
...
impl StreamController {
    ...
    pub(crate) fn finalize(&mut self) -> (Option<Box<dyn HistoryCell>>, Option<String>) {
        let (remaining, source, selection_fragment) = self.core.finalize_remaining();
        if source.is_empty() {
            self.core.reset();
            return (None, None);
        }
