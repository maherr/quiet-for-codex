        ...
        let text = match content {
            ...
            rmcp::model::ContentBlock::Resource(resource) => {
                let uri = match resource.resource {
                    ...
                    _ => return ("<unknown embedded resource>".to_string(), false),
                };
                format!("embedded resource: {uri}")
            }
            rmcp::model::ContentBlock::ResourceLink(link) => format!("link: {}", link.uri),
            _ => format_and_truncate_tool_result(&block.to_string(), max_lines, width),
        };
        let truncated = text.ends_with("...");
        (text, truncated)
    }
...
///   register it as an MCP server via `codex-quiet mcp add`.
